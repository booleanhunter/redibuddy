...
try {
    ...
    if (e.message === 'Index already exists') {
        console.log('📋 Text search index exists already, skipped creation.');
    } else {
        console.error('❌ Error creating text search index:', e);
    }
}

// Create vector search index for embeddings
try {
    await client.ft.create('idx:products:vector', {
        '$.name': {
            type: SCHEMA_FIELD_TYPE.TEXT,
            AS: 'name'
        },
    ...
    if (e.message === 'Index already exists') {
        console.log('🧠 Vector search index exists already, skipped creation.');
    } else {
        console.error('❌ Error creating vector search index:', e);
    }
}
...
export async function loadProductsFromCSV(csvFilePath, batchSize = 50, maxProducts = 1000) {
    try {
        ...
        const existingKeys = await client.keys('noderedis:products:*');
        ...
        for (let i = 0; i < products.length; i += batchSize) {
            ...
            for (let j = 0; j < batch.length; j++) {
                ...
                pipeline.json.set(`noderedis:products:${product.id}`, '$', productData);
